...
# This file is accessible as https://github.com/HyoukaChitandaEru/v2ray-core/blob/master/go.sh
...
downloadV2Ray(){
    ...
    if [[ "${DIST_SRC}" == "jsdelivr" ]]; then
        ...
        DOWNLOAD_LINK="https://github.com/HyoukaChitandaEru/v2ray-core/releases/download/${NEW_VER}/v2ray-linux-${VDIS}.zip"
    fi
...
installV2Ray(){
    ...
    if [[ ! -f "/etc/v2ray/config.json" ]]; then
        ...
        sed -i "s/2233/${PORT}/g" "/etc/v2ray/config.json"
        sed -i "s/1bd01366-a8cb-464f-9a6f-c6f2dd27bfd4/${UUID}/g" "/etc/v2ray/config.json"
